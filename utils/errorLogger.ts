...
// Captures console.log/warn/error and sends to server for debugging
      ...
      fetch(url, {
        ...
        if (!fetchErrorLogged) {
          ...
          if (typeof window !== 'undefined' && window.console) {
            (window.console as any).__proto__.log.call(console, '[TechTimes] Fetch error (will not repeat):', e.message || e);
          }
        }
      });
    } catch (e) {
...
export const setupErrorLogging = () => {
  ...
  originalConsoleLog('[TechTimes] Setting up error logging...');
  originalConsoleLog('[TechTimes] Log server URL:', logServerUrl || 'NOT AVAILABLE');
  originalConsoleLog('[TechTimes] Platform:', Platform.OS);
